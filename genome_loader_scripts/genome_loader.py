...
class GenomeDataFileCreator:
    def __init__(self, contigset_file, gff_file, protein_file, output_dir, run_checkm2_option):
        ...
        self.contigset_hash, self.contig_id_map, self.protein_id_map = self.compute_hash (self.contigset_file,
                                                                                   self.protein_file)
    ...
    def run_bbmap(self, fna_path):
        ...
        try:
            result = subprocess.run(bbmap_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
    ...
    def run_checkm2(self, contigset_path, output_dir):
        ...
        try:
            ...
            with open(tsv_file_path, 'r') as file:
                ...
                if 'Completeness' not in reader.fieldnames or 'Contamination' not in reader.fieldnames:
                    raise ValueError("TSV file does not contain 'Completeness' or 'Contamination' columns.")
                ...
                for row in reader:
                    ...
                    return {"checkm2_contamination": contamination, "checkm2_completeness": completeness}
        ...
        try:
            with open_func(self.gff_file, 'rt', encoding='utf-8', errors='ignore') as file:
                ...
                for row in reader:
                    ...
                    feature_hash = self.generate_hash_id(contigset_hash, contig_hash, start, end, feature_type)
...
class MultiGenomeDataFileCreator:
    ...
    def write_to_tsv(self, contigset, contigs, features, associations, structural_annotation, headers_written):
        ...
        try:
            ...
            with open(sa_file, 'a', newline='') as f_out:
                writer = csv.DictWriter(f_out, fieldnames=['contigset_hash', 'gff_hash', 'contigset_file', 'gff_file'], delimiter='\t')
                if write_header:
                    writer.writeheader()
            ...
            with open(features_file, 'a', newline='') as f_out:
                contig_fieldnames = ['contigset_hash', 'contig_hash','feature_hash', 'feature_type', 'feature_ontology',
            ...
            associations_file = os.path.join(self.output_dir, 'feature_association.tsv')  # Renamed file
        ...
        for gid in genome_ids:
            ...
            if paths:
                ...
                if contigset_file and gff_file and protein_file:
                    contigset, contigs, features, associations, structural_annotation = self.process_files(
                        contigset_file, gff_file, protein_file
                    )

                    # Write data to TSV files incrementally
                    self.write_to_tsv(contigset, contigs, features, associations, structural_annotation, headers_written)
    ...
    def from_args(cls):
        ...
        parser.add_argument('--run_checkm2_option', type=str, default=None, help='Run checkm2, use 1 to run checkm2')

        args = parser.parse_args()

        return cls(genome_paths_file=args.genome_paths_file, output_dir=args.output_dir, run_checkm2_option=args.run_checkm2_option)
