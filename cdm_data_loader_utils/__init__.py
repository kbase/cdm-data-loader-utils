...
class SomeCdmEtl:
    ...
    def export_data_contigset(self):
        ...
        pa_table_contigset = pa.Table.from_arrays([pa.array(_data[k]) for k in names], names=names)

        return pa_table_contigset
    ...
    def export_data_contigs(self):
        ...
        for o in tqdm(self.data_contigs):
            ...
            for _t, _n in o.names:
                _data_names['entity_id'].append(o.hash)
                _data_names['type'].append(_t)
                _data_names['name'].append(_n)

                if _n not in contig_name_to_hash:
                    contig_name_to_hash[_n] = o.hash
                elif contig_name_to_hash[_n] != o.hash:
                    raise ValueError('dup')
        pa_table_contigs = pa.Table.from_arrays([pa.array(_data[k]) for k in names], names=names)
        pa_table_names = pa.Table.from_arrays([pa.array(_data_names[k]) for k in _label_names], names=_label_names)

        return pa_table_contigs, pa_table_names

        # pq.write_table(pa_table, '/scratch/fliu/data/KE/CDM/export/genomes/ke_pangenomes_contigs.parquet')
        # pq.write_table(pa_table_names, '/scratch/fliu/data/KE/CDM/export/genomes/ke_pangenomes_names.parquet')
