...
from src.parsers.uniref import (
    add_cluster_members,
    cdm_entity_id,
    extract_cluster,
    extract_cross_refs,
...
def test_get_timestamps(
    uniref_id: str, existing_created: str, now: dt.datetime, expected: tuple[str] | None
) -> None:
    """Test timestamps."""
...
def test_get_accession_and_seed(
    xml_str: str | None, expected_acc: str | None, expected_is_seed: bool
) -> None:
...
def make_entry_with_members(
    member_xmls: list[str], ns_uri: str = "http://uniprot.org/uniref"
) -> ET.Element:
    """
...
def test_add_cluster_members(
    repr_xml: str | None, member_xmls: list[str], expected: list[tuple[str, ...]]
) -> None:
...
def test_extract_cross_refs_param(
    dbref_props: list[tuple[str, str]], expected_xrefs: list[tuple[str, str]]
) -> None:
    """
