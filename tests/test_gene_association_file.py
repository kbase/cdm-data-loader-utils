...
def test_merge_and_fallback(annotation_df, evidence_mapping_df):
    ...
    assert "DB_Reference" not in merged.columns, (
        "'DB_Reference' should have been dropped after merge."
    )
...
def test_fallback_after_explode():
    annotation_df = pd.DataFrame(
        [{"Evidence_Code": "ND", "publications": ["PMID:111111", "PMID:222222"]}]
    )
    evidence_df = pd.DataFrame(
        [{"Evidence_Code": "ND", "DB_Reference": "DEFAULT", "evidence_type": "ECO:0000307"}]
    )
    ...
    assert all(merged["evidence_type"] == "ECO:0000307"), (
        "Fallback evidence_type not correctly applied after explode"
    )
