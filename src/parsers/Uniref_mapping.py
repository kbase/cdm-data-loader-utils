    ...
    while True:
        ...
        for retry in range(max_retry):
            try:
                ...
                print(
                    f"Exception: {e}, retry {retry + 1}"
                )  ## avoid network and web service issues
                time.sleep(3)
                continue
            if r.ok:
                break
        ...
        for retry in range(max_retry):
            try:
                ...
                if r.ok:
                    ...
                    print(
                        f"[{idx}/{len(ids)}] Failed {cid}, status:{r.status_code}, retry {retry + 1}"
                    )
            except Exception as e:
...
def parse_uniref_xml_to_cdm(xml_file):
    ...
    for entry in root.findall("u:entry", ns):
        ...
        uniref_level = (
            cluster_id.split("_")[0].replace("UniRef", "") if cluster_id else None
        )
...
def flatten_cdm_entry(cdm):
    ...
    # Represents a member attribute, prefixed with rep_ to avoid conflicts with member fields
    rep = cdm.get("representative", {})
    rep_flat = {f"rep_{k}": v for k, v in rep.items()}

    records = []
    # Iterate over all members, output one line per member
