...
python refseq_api.py \
  ...
  --debug \
  --unique-per-taxon \
  --data-dir /Users/yuewang/Documents/RefSeq/entities_data
...
def write_delta(
    ...
    """
    Write Pandas DataFrame to Delta. If data_dir is provided, write to external LOCATION
    at {data_dir}/{database}/{table} and register the table. Otherwise use managed table.
    """
    ...
    if table == "contig_collection":
        ...
        pandas_df = pandas_df.astype(object).where(pandas_df.notnull(), None)
